...
const EditProductForm = ({product}: EditProductFormProps) => {
  ...
  useEffect(() => {
    setImages(productImages as IImage[])
  }, [productImages])
  ...
  const onImageDelete = (index: number) => {
    ...
    const isPreviewImage = typeof foundImage === 'string'
    if (!isPreviewImage) {
      setDeletedImages((state) => [...state, foundImage as IImage])
    }
  ...
  const sendFormData = (data: any) => {
    if (!isLoading) {
      ...
      deletedImages.forEach((image) => {
        formData.append('deletedImages', image.public_id)
      })
